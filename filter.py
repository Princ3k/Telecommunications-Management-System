...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        false = False
        try:
            customer = int(filter_string) # customer ID
            for c in customers:
                if c.get_id() == customer:
                    numbers = c.get_phone_numbers()
                    false = True
            if not false:  # filter string invalid
                return data
            else:
                for call in data:
                    if call.src_number in numbers:
                        calls.append(call)
                    elif call.dst_number in numbers:
                        calls.append(call)
                return calls
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        calls = []  # create list of calls
        try:
            c = filter_string[0]  # creating a char

            if c not in ('L', 'G'):
                return data
            seconds = int(filter_string[1:])   # creating number of seconds

            if c == 'L':
                for call in data:
                    if call.duration < seconds:
                        calls.append(call)
            else:
                for call in data:
                    if call.duration > seconds:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            ...
            lower_long = int(lower_long)
            lower_lat = int(lower_lat)
            upper_long = int(upper_long)
            upper_lat = int(upper_lat)
            for call in data:
