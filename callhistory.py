...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        history = (call.time.month, call.time.year)
        if history in self.outgoing_calls:
            self.outgoing_calls[history].append(call)
        else:
            self.outgoing_calls[history] = [call]

    def register_incoming_call(self, call: Call) -> None:
        """ Register a Call <call> into this incoming call history
        """
        history = (call.time.month, call.time.year)
        if history in self.incoming_calls:
            self.incoming_calls[history].append(call)
        else:
            self.incoming_calls[history] = [call]
