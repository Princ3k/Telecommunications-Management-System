...
from contract import Contract, MTMContract, TermContract, PrepaidContract
...
def process_event_history(log: dict[str, list[dict]],
    ...
    #
    for data in log['events']:
        date = datetime.datetime.strptime(data['time'], "%Y-%m-%d %H:%M:%S")
        if date.month != billing_month or date.year != billing_year:
            new_month(customer_list, date.month, date.year)

        if data['type'] == 'call':
            call_time = datetime.datetime.strptime(data['time'],
                                                   "%Y-%m-%d %H:%M:%S")

            call = Call(data['src_number'], data['dst_number'], call_time,
                        data['duration'],
                        data['src_loc'], data['dst_loc'])

            src_customer = find_customer_by_number(data['src_number'],
                                                   customer_list)
            dst_customer = find_customer_by_number(data['dst_number'],
                                                   customer_list)
            src_customer.make_call(call)
            dst_customer.receive_call(call)
